...
// QueryPets queries the pets edge of a Car.
func (c *CarClient) QueryPets(ca *Car) *PetQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, car.PetsTable, car.PetsColumn),
		)
		fromV = sqlgraph.Neighbors(ca.driver.Dialect(), step)
		return fromV, nil
	}
...
type (
	hooks struct {
		Car, Card, Group, Pet, User []ent.Hook
	}
	inters struct {
		Car, Card, Group, Pet, User []ent.Interceptor
	}
)
